...
const PORT = Number(flag('--port') || process.env.GMN_PORT || 7040);
...
const COMPANY_FILE = process.env.GMN_COMPANY_FILE || path.join(ROOT, 'gmn-company.json');
const SESSION_FILE = process.env.GMN_SESSION_FILE || path.join(ROOT, 'gmn-sessions.json');
const CHAT_FILE = process.env.GMN_CHAT_FILE || path.join(ROOT, 'gmn-chat.json');
const DM_FILE = process.env.GMN_DM_FILE || path.join(ROOT, 'gmn-dms.json');
const FILES_FILE = process.env.GMN_FILES_FILE || path.join(ROOT, 'gmn-files.json');
...
const FILES_DIR = process.env.GMN_FILES_DIR || path.join(ROOT, 'gmn-files');

/* Big enough for a phone photo of a CMR note, small enough that one driver
   cannot fill the disk by holding the button down. */
const MAX_UPLOAD = Number(process.env.GMN_MAX_UPLOAD || 25 * 1024 * 1024);
const SITE_DIR = process.env.GMN_SITE_DIR || ROOT;
...
const OWN_COMPANY = !process.env.GMN_COMPANY_FILE;
...
const STUN = (process.env.GMN_STUN_URLS
  || 'stun:stun.l.google.com:19302,stun:stun1.l.google.com:19302')
  .split(',').map((s) => s.trim()).filter(Boolean);

const ICE_SERVERS = [{ urls: STUN }].concat(
  process.env.GMN_TURN_URL
    ? [{
      urls: process.env.GMN_TURN_URL.split(',').map((s) => s.trim()).filter(Boolean),
      username: process.env.GMN_TURN_USER || undefined,
      credential: process.env.GMN_TURN_PASS || undefined,
    }]
    : []);

/* ---------------- small helpers ---------------- */
/* The state files moved with the company name: gmn-company.json is
...
function legacyName(file) {
  ...
  return path.join(dir, 'gmn-' + base.slice(4));
}
...
const ROOM_NAMES_FILE = process.env.GMN_ROOM_NAMES_FILE
...
const ROOM_READS_FILE = process.env.GMN_ROOM_READS_FILE
...
async function api(req, res, url) {
  ...
  if (p === '/api/files' && method === 'POST') {
    ...
    const name = String(req.headers['x-gmn-filename']
        || req.headers['x-gmn-filename'] || 'file' + ext)
      .replace(/[\r\n]/g, '').slice(0, 120);
