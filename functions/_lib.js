...
/* The builds, by the key the page uses. Kept here rather than read from
   ...
   The two names have to agree with CLIENT_RELEASE in script.js, and they
   had drifted twice over while nothing was watching: this said
   Gaming-Nation-Trucker-1.0.0 long after the release files were renamed
...
export const RELEASE_VERSION = '1.1.9';
const OBJECT = (suffix) => `Gaming-Nation-Tracker-${RELEASE_VERSION}-${suffix}`;

export const BUILDS = {
  'win-setup': {
    object: OBJECT('windows-setup.exe'),
    type: 'application/octet-stream',
  },
  'win-portable': {
    object: OBJECT('windows-portable.exe'),
    type: 'application/octet-stream',
  },
  android: {
    object: OBJECT('android.apk'),
