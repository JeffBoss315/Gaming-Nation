...
const port = Number(flag('port', process.env.GMN_TELEMETRY_PORT || 25555));
const map = flag('map', process.env.GMN_TELEMETRY_MAP || 'Local\\GMNTelemetry');
const adapter = flag('adapter', process.env.GMN_TELEMETRY_ADAPTER ||
  path.join(__dirname, 'gmn-telemetry-adapter.exe'));
const plugin = flag('plugin', process.env.GMN_TELEMETRY_PLUGIN ||
...
const metadata = flag('metadata', process.env.GMN_TELEMETRY_METADATA ||
