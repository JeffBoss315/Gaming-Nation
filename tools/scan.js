...
/* 9b. and the gate in front of them must name the same files.

      functions/_lib.js holds the only list of objects a signed link can
      ever point at — a key that is not in it cannot be downloaded. It is a
      second copy of the release names, and it drifted twice without a word:
      it still said Gaming-Nation-Trucker-1.0.0 after the files were renamed
      to Gaming-Nation-Tracker- and the version had reached 1.1.6, so every
      link the gate minted pointed at an object that is not in the bucket.

      Nothing caught it because the test that covers the gate could not run
      at all — functions/ had been deleted by accident in an unrelated
      commit, and `npm run smoke:downloads` died on the missing import. */
(() => {
  const libPath = path.join(ROOT, 'functions', '_lib.js');
  if (!fs.existsSync(libPath)) {
    fail('downloads', 'the download gate is gone — the site has no /api/download-link',
      'functions/_lib.js');
    return;
  }
  const lib = fs.readFileSync(libPath, 'utf8');
  const ver = (lib.match(/RELEASE_VERSION\s*=\s*'([^']+)'/) || [])[1];
  const stem = (lib.match(/OBJECT\s*=\s*\([^)]*\)\s*=>\s*`([^`]*)`/) || [])[1];
  if (!ver || !stem) {
    fail('downloads', 'scan cannot find the build names in the gate', 'functions/_lib.js');
    return;
  }
  if (ver !== pkg.version) {
    fail('downloads', 'the gate is on ' + ver + ' but package.json says ' + pkg.version
      + ' - every link it mints points at the wrong build', 'functions/_lib.js');
    return;
  }
  const named = uniq(all(lib, /OBJECT\('([^']+)'\)/g)
    .map((m) => stem.replace('${RELEASE_VERSION}', ver).replace('${suffix}', m[1])));
  const offered = listed.map(([, f]) => f.split('/').pop());

  offered.filter((f) => !named.includes(f))
    .forEach((f) => fail('downloads', 'the download page offers a build the gate cannot serve', f));
  named.filter((f) => !offered.includes(f))
    .forEach((f) => fail('downloads', 'the gate names a build the page does not offer', f));

  if (offered.every((f) => named.includes(f)) && named.length === offered.length) {
    console.log(`  the gate serves the same ${named.length} build(s) the page offers`);
  }
})();
...
(function versionAgrees() {
  ...
  /* The Android app carries its own copy of the number, and it was never
     touched: every APK we shipped said versionName "1.0", versionCode 1
     inside, whatever the filename claimed. build-apk.js stamps it now, so
     this only has to catch the gradle file being edited back by hand or
     regenerated by Capacitor. */
  const gradle = path.join(ROOT, 'android', 'app', 'build.gradle');
  if (!fs.existsSync(gradle)) return;
  const g = fs.readFileSync(gradle, 'utf8');
  const name = g.match(/versionName\s+"([^"]*)"/);
  if (name && name[1] !== pkg.version) {
    fail('android', 'versionName says ' + name[1] + ' but package.json says '
      + pkg.version + ' - the APK reports the wrong build',
      'android/app/build.gradle');
    return;
  }
  console.log('  the Android project agrees too: ' + (name ? name[1] : '?'));
})();
