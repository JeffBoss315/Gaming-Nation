...
const PORT = Number(process.env.GMN_SMOKE_PORT || 7094);
...
const COMPANY = {
  drivers: [
    ...
    { id: 'GMN-2002', name: 'Marek Kowal', role: 'driver', accountStatus: 'active' },
    { id: 'GMN-3003', name: 'Nosy Parker', role: 'driver', accountStatus: 'active' },
  ],
  accounts: [
    { driverId: 'GMN-1001', email: 'anna@example.com', salt: SALT, hash: sha(PW, SALT) },
    { driverId: 'GMN-2002', email: 'marek@example.com', salt: SALT, hash: sha(PW, SALT) },
    { driverId: 'GMN-3003', email: 'nosy@example.com', salt: SALT, hash: sha(PW, SALT) },
  ],
  events: [],
  jobs: [],
};
...
(async () => {
  ...
  const sent = await req('POST', '/api/dm/send',
    { to: 'GMN-2002', text: 'Are you running the Rotterdam load?' }, anna);
  ...
  check('the sender cannot label themselves',
    forged.status === 200 && forged.body.message.driverId === 'GMN-2002'
  ...
  const empty = await req('POST', '/api/dm/send', { to: 'GMN-2002', text: '   ' }, anna);
  check('an empty message is not a message', empty.status === 400, 'HTTP ' + empty.status);

  /* ---------- 3. the thread reads back ---------- */
  const thread = await req('GET', '/api/dm/GMN-2002', null, anna);
  ...
  const withFile = await req('POST', '/api/dm/send',
    { to: 'GMN-2002', text: '', attachment: { id: up.body.file.id } }, anna);
  ...
  const ring = await req('POST', '/api/call/signal',
    { to: 'GMN-2002', kind: 'ring', callId: 'CALL-1', video: false }, anna);
  ...
  await req('POST', '/api/call/signal',
    { to: 'GMN-2002', kind: 'offer', callId: 'CALL-1', payload: { sdp: 'v=0 offer' } }, anna);
  await req('POST', '/api/call/signal',
    { to: 'GMN-1001', kind: 'answer', callId: 'CALL-1', payload: { sdp: 'v=0 answer' } }, marek);
  await req('POST', '/api/call/signal',
    { to: 'GMN-2002', kind: 'ice', callId: 'CALL-1', payload: { candidate: 'a=candidate:1' } }, anna);
  await wait(400);
  ...
  const offline = await req('POST', '/api/call/signal',
    { to: 'GMN-3003', kind: 'ring', callId: 'CALL-2' }, anna);
  sNosy.stop();
  await wait(300);
  const nowOffline = await req('POST', '/api/call/signal',
    { to: 'GMN-3003', kind: 'ring', callId: 'CALL-3' }, anna);
  ...
  const nonsense = await req('POST', '/api/call/signal',
    { to: 'GMN-2002', kind: 'drop-tables', callId: 'CALL-1' }, anna);
  ...
  check('and the one already in hears them arrive',
    annaCall.some((e) => e.data.kind === 'room.joined' && e.data.from === 'GMN-2002'),
  ...
  check('and the room hears it',
    sAnna.got.filter((e) => e.kind === 'call')
      .some((e) => e.data.kind === 'room.left' && e.data.from === 'GMN-2002'),
    'yes');
  ...
  const after = await req('GET', '/api/dm/GMN-2002', null, again);
