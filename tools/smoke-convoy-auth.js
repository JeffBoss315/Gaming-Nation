...
const PORT = Number(process.env.GMN_SMOKE_PORT || 7092);
...
const COMPANY = {
  drivers: [
    ...
    { id: 'GMN-2002', name: 'Ordinary Driver', role: 'driver', accountStatus: 'active',
      km: 0, deliveries: 0, earned: 0 },
    { id: 'GMN-3003', name: 'Outside Driver', role: 'driver', accountStatus: 'active',
      km: 0, deliveries: 0, earned: 0 },
  ],
  accounts: [
    { driverId: 'GMN-1001', email: 'leader@example.com', salt: SALT_A, hash: sha(PW_A, SALT_A) },
    { driverId: 'GMN-2002', email: 'driver@example.com', salt: SALT_B, hash: sha(PW_B, SALT_B) },
    { driverId: 'GMN-3003', email: 'outside@example.com', salt: SALT_B, hash: sha(PW_B, SALT_B) },
  ],
  events: [
    { id: 'EV-7001', name: 'Auth Convoy', status: 'live', leaderId: 'GMN-1001',
      ...
      path: ['Rotterdam', 'Hamburg'], registered: [
        ...
        { driverId: 'GMN-2002', state: 'confirmed' },
      ], activity: [] },
  ],
  jobs: [],
};
...
(async () => {
  ...
  check('a token identifies its holder',
    me.status === 200 && me.body.id === 'GMN-2002', me.body ? me.body.id : '-');
  ...
  check('but the identity comes from the token, not the body',
    !!spoofed && spoofed.driverId === 'GMN-2002' && spoofed.driver === 'Ordinary Driver',
  ...
  /* ---------- 4. moderation ---------- */
  const mine = (afterLeader.body.messages || []).find((m) => m.driverId === 'GMN-2002');
