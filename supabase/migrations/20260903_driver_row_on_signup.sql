...
--
-- The first version of this handed out GMN7001, GMN7002, ... by row
      ...
      for n in 1000..9999 loop
        if not exists (
          select 1 from public.drivers
           where driver_code = 'GMN' || lpad(n::text, 4, '0')
        ) then
          final_code := 'GMN' || lpad(n::text, 4, '0');
          exit;
        end if;
      end loop;
    end if;
...
create or replace function public.gmn_signup_ready()
returns boolean
language sql
security definer
set search_path = public
...
grant execute on function public.gmn_signup_ready() to anon, authenticated;
