-- ============================================================
-- GAMING NATION — new driver codes are GMN, not GMN
--
...
-- in 20260903_driver_row_on_signup.sql still issued GMN####, and
...
-- the browser finished got an GMN code while the page believed it
...
--
-- Every driver already issued an GMN code keeps it. Their code is
...
--
--   GMN-1001, GMN4821, ...   everyone who was already here
--   GMN3907, ...             everyone from now on
--
-- The site reads both. script.js matches /^(GMN|GMN)/i where it
...
begin
  ...
  -- reloaded since the rename still sends GMN####, and refusing it would
  ...
  -- can never collide with an issued GMN one either.
...
--
-- public.gmn_signup_ready() keeps its name. It is a diagnostic
...
-- 3. Check it
--
--   -- the function should mention GMN and not GMN
--   select prosrc like '%GMN%' as mints_gmn,
--          prosrc like '%''GMN''%' as still_mints_gmn
